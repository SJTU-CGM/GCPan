...
<div class="page-header">
   <h1>Shared Gene Information of the Rice Accessions</h1>
</div>

<?php 
	error_reporting(E_ALL^E_NOTICE^E_WARNING);
	if (isset($_POST['rname']) or isset($_POST['selection'])) {
		...
		if (isset($_POST['rnum'])) {
			...
			$core_num = count($code_arr);
		}
		$err_code = array();
		$trim_arr = array();
		foreach ($code_arr as $code) {
			array_push($trim_arr, trim($code));
		}
		$code_arr = $trim_arr;
		foreach ($code_arr as $code) {
			...
			if ($check_rs[0] == 0) {
				array_push($err_code, $code);
			}
		}
		if (!empty($err_code)){
			echo "<p>Wrong query input: ".implode(', ', $err_code).", please input the rice code which was showed in the <a href=\"rice-table.php\">rice table</a></p>";
		}
		else{
			$rice_sql = "SELECT * FROM `line_info` where `line_code` IN ('".implode("','", $code_arr)."')";
			...
			$tmp_sql = "SELECT `gene_no` from `gene_existence` where `line_code` IN ('".implode("','", $code_arr)."')";

			$q1 = mysql_query($tmp_sql);
			$tmp_array = array();
			while ($rs = mysql_fetch_assoc($q1)) {
				if (array_key_exists($rs['gene_no'], $tmp_array)){
					$tmp_array[$rs['gene_no']] += 1;
				}else{
					$tmp_array[$rs['gene_no']] = 1;
				}
			}
			...
			$real_sql = "SELECT * FROM `gene_info` WHERE `gene_no` IN (".implode(',', $tmp_array2).")";
			...
			$random_gene_num = 0;
			$gene_feature_sql = "SELECT * FROM `gene_feature` WHERE `gene_no` IN (".implode(',', $tmp_array2).")";
